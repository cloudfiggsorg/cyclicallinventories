...
public class SapOperationDao {
	...
	private static final String GET_E_LQUA_FOR_WM_LANES = "SELECT EL.LGNUM, SUBSTRING(EL.MATNR, PATINDEX('%[^0 ]%', EL.MATNR + ' '), LEN(EL.MATNR)) MATNR, "
			+ " T6.MSEH3 MEINS, EL.LGORT, EL.LGTYP, EL.LGPLA, TL.LGOBE, MK.MAKTX, SUM(CONVERT(NUMERIC(23,3),EL.VERME)) VERME FROM E_LQUA EL WITH(NOLOCK) "
			+ " INNER JOIN T001L TL WITH(NOLOCK) ON EL.WERKS = TL.WERKS AND EL.LGORT = TL.LGORT "
			+ " INNER JOIN T006A T6 WITH(NOLOCK) ON EL.MEINS = T6.MSEHI"
			+ " INNER JOIN MAKT MK WITH(NOLOCK) ON EL.MATNR = MK.MATNR WHERE DOC_INV_ID = ? "
