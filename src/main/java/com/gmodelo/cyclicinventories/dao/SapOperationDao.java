...
public class SapOperationDao {
	...
	private static final String GET_MBEW_PIVOT = "SELECT DIP_MATNR MATNR FROM (SELECT DIP_MATNR FROM INV_DOC_INVENTORY_POSITIONS WITH(NOLOCK) WHERE DIP_DOC_INV_ID = ? GROUP BY DIP_MATNR " 
			+ "UNION SELECT MATNR FROM INV_VW_GET_EXP_MAT_FOR_DOC_INV WITH(NOLOCK) WHERE DOC_INV_ID = ? " 
			+ "UNION SELECT DIP_VHILM FROM INV_DOC_INVENTORY_POSITIONS WITH(NOLOCK) WHERE DIP_DOC_INV_ID = ? GROUP BY DIP_VHILM)	AS A " 
			+ "WHERE DIP_MATNR NOT IN(SELECT MATNR FROM INV_CIC_E_PIV_MBEW WITH(NOLOCK)) UNION " 
			+ "SELECT A.MATNR FROM INV_CIC_E_PIV_MBEW AS A WITH(NOLOCK) "
			+ "INNER JOIN (SELECT DISTINCT(SUBSTRING(MATNR, PATINDEX('%[^0 ]%', MATNR + ' '), LEN(MATNR))) MATNR FROM E_LQUA WITH(NOLOCK) WHERE DOC_INV_ID = ? " 
			+ "UNION SELECT DISTINCT(SUBSTRING(MATNR, PATINDEX('%[^0 ]%', MATNR + ' '), LEN(MATNR))) MATNR FROM E_MARD WITH(NOLOCK) WHERE DOC_INV_ID = ?) AS B " 
