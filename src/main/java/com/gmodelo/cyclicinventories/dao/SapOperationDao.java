...
public class SapOperationDao {
	...
	private static final String GET_MBEW_PIVOT = "SELECT MATNR from INV_CIC_E_PIV_MBEW WITH(NOLOCK) "
			...
			+ " AND (MATNR IN (SELECT DIP_MATNR from INV_DOC_INVENTORY_POSITIONS WITH(NOLOCK) WHERE DIP_DOC_INV_ID = 236 GROUP BY DIP_MATNR "  
			+ " UNION SELECT MATNR FROM INV_VW_GET_EXP_MAT_FOR_DOC_INV WHERE DOC_INV_ID = ? "
			+ " UNION SELECT DIP_VHILM from INV_DOC_INVENTORY_POSITIONS WITH(NOLOCK) WHERE DIP_DOC_INV_ID = ? GROUP BY DIP_VHILM " 
			+ " UNION SELECT DISTINCT(SUBSTRING(MATNR, PATINDEX('%[^0 ]%', MATNR + ' '), LEN(MATNR))) MATNR FROM E_LQUA WHERE DOC_INV_ID = ? "
			+ " UNION SELECT DISTINCT(SUBSTRING(MATNR, PATINDEX('%[^0 ]%', MATNR + ' '), LEN(MATNR))) MATNR FROM E_MARD WHERE DOC_INV_ID = ?)) ";
